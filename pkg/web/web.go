...
func (server Server) HandleRequests() {
	...
	r.HandleFunc("/api/v1/notes", server.ReturnAllNotes)
	r.HandleFunc("/api/v1/note/{id}", server.ReturnSingleNote)
	r.HandleFunc("/api/v1/note", server.CreateNewNote)
	r.HandleFunc("/api/v1/note/{id}/{userid}", server.UpdateNote)
	r.HandleFunc("/api/v1/note/{id}", server.DeleteNote)
	r.HandleFunc("/api/v1/note/{id}/{targetFunction}/{prefix}", server.SearchNotesForSpecifics)

	r.HandleFunc("/api/v1/users", server.ReturnAllUsers)
	r.HandleFunc("/api/v1/user/{id}", server.ReturnSingleUser)
	r.HandleFunc("/api/v1/user/name/{username}", server.ReturnSingleUserByName)
	r.HandleFunc("/api/v1/user/email/{email}", server.ReturnSingleUserByEmail)
	r.HandleFunc("/api/v1/user", server.CreateNewUser)
	r.HandleFunc("/api/v1/user/{id}", server.UpdateUser)
	r.HandleFunc("/api/v1/user/{id}", server.DeleteUser)

	r.HandleFunc("/api/v1/usersettings/{id}", server.ReturnSingleUserSettings)
	r.HandleFunc("/api/v1/usersettings", server.CreateNewUserSettings)
	r.HandleFunc("/api/v1/usersettings/{id}", server.UpdateUserSettings)
	r.HandleFunc("/api/v1/usersettings/{id}", server.DeleteUserSettings)

	r.HandleFunc("/api/v1/usersnotes/{id}", server.GetAllNotesUserHasAccessTo)
	r.HandleFunc("/api/v1/login/{username}/{password}", server.Login)

	r.HandleFunc("/api/v1/", server.CheckConnection)
